...
import org.sample.park.StatusRequest;  // Add this import
...
public class CarParkGrpcClient {
    ...
    public String getStatus() {
        StatusRequest request = StatusRequest.newBuilder().build();  // Empty request
        logger.info("Sending status request to gRPC server");
        try {
            CommandResponse response = blockingStub.getStatus(request);
            logger.info("Received status from gRPC server: {}", response.getResult());
            return response.getResult();
        } catch (StatusRuntimeException e) {
            logger.error("gRPC status request failed: {}", e.getStatus(), e);
            return "gRPC status request failed: " + e.getStatus();
        } catch (Exception e) {
            logger.error("Unexpected error during gRPC status request", e);
            return "Unexpected error: " + e.getMessage();
        }
    }
}
